...
export default function LeagueStats() {
  ...
  const averageRating = (
    (data[0].value + 2 * data[1].value + 3 * data[2].value) /
    6
  ).toFixed(1);
    ...
    <section className="grid md:grid-cols-2 gap-4 my-4">
      <div className="bg-white shadow-lg rounded-md grid grid-cols-2 py-4 gap-8">
        {data.map(({ name, value }: any, index) => (
          <div
            className="flex flex-col justify-between items-between p-4 py-4"
            key={index}
          >
      ...
      <div className="bg-white shadow-lg rounded-md px-4 py-8">
        <h2 className="font-[700] text-[1.5rem] text-center lg:text-left">
          Average Ratings
        </h2>
        <div className="grid justify-center items-center lg:grid-cols-2 gap-4">
          <MyPieChart
            data={data}
            colors={colors}
            averageRatingsValue={averageRating}
          />
          <div className="p-4">
